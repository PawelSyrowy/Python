...
def update_characters_left(number_of_characters):
    ...
    if number_of_characters < 0 or number_of_characters > characters_left:
        print("Liczba znaków z spoza przedziału 0,", characters_left)
        sys.exit(0)
    else:
...
#walidacja, czy użytkownik podaje liczbe, poproszenie o podaną liczbe
# teraz wyłączamy program w powinniśmy dać możliwość spróbować ponownie
# również kiedy poda hasło krótsze niż 5, program się wyłącza, a nie daje możliwości spróbować jeszcze raz

#HASŁO W JEDNEJ LINII KODU
